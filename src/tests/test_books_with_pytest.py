...
async def test_create_book(async_client, db_session):
    ...
    data = {"title": "Wrong Code", "seller_id": seller.id, "author": "Robert Martin", "pages": 104, "year": 2007}
...
async def test_get_books(db_session, async_client):
    ...
    assert response.json() == {
        "books": [
            {"title": "Eugeny Onegin", "seller_id": seller.id, "author": "Pushkin", "year": 2001, "id": book.id,
             "count_pages": 104},
            {"title": "Mziri", "author": "Lermontov", "seller_id": seller.id, "year": 1997, "id": book_2.id,
             "count_pages": 104},
        ]
    }
...
async def test_get_single_book(db_session, async_client):
    ...
    # Создаем книги с корректным seller_id
    book = books.Book(author="Pushkin", title="Eugeny Onegin", year=2001, count_pages=104, seller_id=seller.id)
    book_2 = books.Book(author="Lermontov", title="Mziri", year=1997, count_pages=104, seller_id=seller.id)
    ...
    assert response.json() == {
        ...
        "count_pages": 104,
        "id": book.id,
        "seller_id": seller.id  # Use the correct seller_id from the seller object
    }
...
async def test_update_book(db_session, async_client):
    ...
    response = await async_client.put(
        ...
        json={"title": "Mziri", "seller_id": seller.id, "author": "Lermontov", "count_pages": 100, "year": 2007,
              "id": book.id},
    )
